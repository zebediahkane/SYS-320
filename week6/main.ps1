...
while($operation){
    ...
    elseif($choice -eq 3){ 
        ...
        if (checkUser($name)) {
            Write-Host "Username already exists" | Out-String
        } else {
            ...
            if (checkPassword($password)) {
                ...
                Write-Host "Password must contain special charicters" | Out-String
            }
        }
    }
    ...
    elseif($choice -eq 7){
        ...
        if (checkUser($name)) {
            $days = Read-Host -Prompt "Please enter number of days back"
